...
public class B_JpaDaoTest {
   ...
   * TESTS
    ...
    public void h_testerRechercher() throws MyDBException {
        System.out.println("*** " + SystemLib.getCurrentMethod() + " ...");
        Boolean test = false;
        List<Personne> listetest = persWrk.lireListe();
        Personne p = listetest.get(0);
        if (p != null) {
            Personne o = persWrk.rechercher("nom", p.getNom());
            System.out.println(" Personne recherchée = " + p.toString());
            System.out.println(" Personne trouvée = " + o.toString());
            test = true;
        }
        assertTrue(test);
    }
